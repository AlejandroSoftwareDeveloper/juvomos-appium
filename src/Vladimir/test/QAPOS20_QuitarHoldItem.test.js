...
// Flow
const tapUnHoldItemFlow = require('../flows/TapUnHoldItemFlow');
...
describe('Flujo: Hold en item', function () {
    ...
    before(async function () {
        ...
        if (typeof tapUnHoldItemFlow !== 'function') {
            throw new Error('TapHoldItemFlow no es un constructor. Revisa module.exports del Flow.');
        }
        ...
        tapunholdFlow = new tapUnHoldItemFlow(
            menuPage,
            paymentPage,
            holdPage
        );
    ...
    it('Quitar hold a item: Agregar 2 items Poner el primer en Hold Click print Quitar el hold del primer item', async function () {
        try {
            // ------------------------------
            // Ejecuta el flujo funcional completo (pasos 1 a 4)
            // ------------------------------
            await tapunholdFlow.execute();
            await tapunholdFlow.execute();
