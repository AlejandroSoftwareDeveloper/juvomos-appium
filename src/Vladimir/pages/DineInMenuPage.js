...
class DineInMenuPage extends BasePage {
    ...
    async isDisplayed() {
        try {
            ...
            const el = await driver.$(`id=com.juvomos.pos:id/${this.showOrderButton}`);
    ...
    //El codigo que se comenta Crea inestabilidad y caídas de UiAutomator2
    //porque Ignora anchorSelector, no espera (waitForExist),
    //cambia el criterio respecto a todas las demás páginas,
    //Si el botón aparece tarde → devuelve false,
    // Esto desencadena flujos que siguen cuando no deben
    /*
    async isDisplayed() {
        try {
            const driver = await this.getDriver();
    ...
    */
    ...
    async selectProductByName(productName) {
        ...
        // XPath en una sola línea (Appium es sensible a saltos de línea)
        ...
        const productCard = await driver.$(productCardSelector);

        // Espera segura a que exista (no solo isDisplayed)
        const exists = await productCard.waitForExist({ timeout: 10000 }).catch(() => false);
        if (!exists) {
            throw new Error(`Producto no encontrado: ${productName}`);
        }

        // isDisplayed protegido
        const visible = await productCard.isDisplayed().catch(() => false);
        if (!visible) {
...
*/
    ...
    async openOrderSafe() {
        ...
        `id=com.juvomos.pos:id/${this.showOrderButton}`
        );
...
/*
    ...
    try {
        ...
        const button = await driver.$(
            `android=new UiSelector().resourceId("com.juvomos.pos:id/${this.showOrderButton}")`
        );
        ...
        // VALIDACIÓN REAL DE NAVEGACIÓN
        await this.driver.waitUntil(
            async () => await this.paymentPage.isDisplayed(),
            {
                timeout: 10000,
                timeoutMsg: 'No se navegó a la pantalla de pago después de abrir la cuenta'
            }
        );
